			 ...
			 input_result *result)
{
	...
	return;
	sprintf(buf,"sip:%s@10.5.3.172",uname);

	printf("will call party  %s",buf);
 ...
 void qt_make_new_call(char * calledname)
{
   ...
   //ui_input_url("Make call", buf, sizeof(buf), &result);
    sprintf(buf,"sip:%s@10.5.43.172",calledname);
	result.uri_result = buf;
	result.nb_result = PJSUA_APP_NO_NB;
...
//1 hangup all
static void qt_hangup_call(int ha)
{
    if (current_call == -1) {
...
static void ui_add_buddy()
{
 ...
 void qt_add_account(char *username)
{
	...
	strcpy(registrar,"sip:10.5.43.172");
	strcpy(realm,"*");
	strcpy(passwd,"sip123");
	sprintf(id,"sip:%s@10.5.3.172",uname);

    printf("Your SIP URL:%s\r\n", id);
    printf("URL of the registrar:%s\r\n", registrar);
    printf("Auth Realm:%s\r\n", realm);
...
static void ui_add_account(pjsua_transport_config *rtp_cfg)
{
   ...
   //return;
	...
	printf("Your SIP URL:%s", id);
   	printf("URL of the registrar:%s", registrar);
